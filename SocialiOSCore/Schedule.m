...
static NSString* TAG = @"SOCIAL Schedule";
...
- (id)initWithDictionary:(NSDictionary *)dict {
    if (self = [super init]) {
        if (dict[SOCIAL_SCHE_REC]) {
            recurrence = [dict[SOCIAL_SCHE_REC] intValue];
        } else {
            recurrence = NONE;
        }
        activationLimit = [dict[SOCIAL_SCHE_APPROVALS] intValue];
        
        NSMutableArray* tmpTimeRanges = [NSMutableArray array];
        if (dict[SOCIAL_SCHE_RANGES]) {
            NSArray* rangesObjs = [dict[SOCIAL_SCHE_RANGES] array];
            for(NSDictionary* trDict in rangesObjs) {
                long long tmpTimeMillis = [trDict[SOCIAL_SCHE_RANGE_START] longLongValue];
                NSDate* start = [NSDate dateWithTimeIntervalSince1970:(tmpTimeMillis/1000.0f)];
                tmpTimeMillis = [trDict[SOCIAL_SCHE_RANGE_END] longLongValue];
...
- (NSDictionary *)toDictionary {
    ...
    if (timeRanges) {
        for(DateTimeRange* dtr in timeRanges) {
            ...
            [tmpTimeRanges addObject:@{
                                       SOCIAL_CLASSNAME: [SocialUtils getClassName:dtr],
                                       SOCIAL_SCHE_RANGE_START: [NSNumber numberWithLongLong:startMillis],
                                       SOCIAL_SCHE_RANGE_END: [NSNumber numberWithLongLong:endMillis]
                                       }];
        }
    }
    ...
    return @{
             ...
             SOCIAL_SCHE_REC: [NSNumber numberWithInt:recurrence],
             SOCIAL_SCHE_RANGES: tmpTimeRanges,
             SOCIAL_SCHE_APPROVALS: [NSNumber numberWithInt:activationLimit]
             };
}
