...
+ (NSString*)getValueForKey:(NSString*)key {
    key = [SocialEncryptor encryptString:key];
    NSString* val = [[self kvDatabase] getValForKey:key];
    if (val && [val length]>0){
        return [SocialEncryptor decryptToString:val];
    }
...
+ (void)setValue:(NSString*)val forKey:(NSString*)key {
    key = [SocialEncryptor encryptString:key];
    [[self kvDatabase] setVal:[SocialEncryptor encryptString:val] forKey:key];
}
...
+ (NSDictionary*)getKeysValuesForNonEncryptedQuery:(NSString*)query {
    ...
    for (NSString* key in keys) {
        ...
        if (val && [val length]>0){
            NSString* valDec = [SocialEncryptor decryptToString:val];
...
+ (NSArray *)getEncryptedKeys {
    ...
    for (NSString *encryptedKey in encryptedKeys) {
        @try {
            NSString *unencryptedKey = [SocialEncryptor decryptToString:encryptedKey];
            if (unencryptedKey) {
...
static NSString* TAG = @"SOCIAL KeyValueStorage";
